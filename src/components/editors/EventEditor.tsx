...
import type { Entity, Event } from '@/types/entities'

interface EventFormData extends Record<string, unknown> {
  summary: string
  dtstart: string // HTML datetime-local format
  dtend: string
  location: string
  description: string
  calendarId: string
...
function toFormData(entity: Entity): EventFormData {
  const e = entity as Event
  return {
    summary: e.summary,
    dtstart: icalToHtmlDatetime(e.dtstart),
    dtend: icalToHtmlDatetime(e.dtend),
    location: e.location ?? '',
...
function toEntity(formData: EventFormData, original: Entity): Event {
  const e = original as Event
  return {
    ...e,
    summary: formData.summary,
    dtstart: htmlDatetimeToIcal(formData.dtstart),
    dtend: htmlDatetimeToIcal(formData.dtend),
...
export function EventEditor({ event }: { event: Event }) {
  ...
  const form = useEditorForm<EventFormData>({
    entityType: 'event',
    uid: event.uid,
    toFormData,
    toEntity,
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-border px-4 py-2">
        <h2 className="text-sm font-semibold text-muted-foreground">Edit Event</h2>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Are you sure you want to delete &ldquo;{event.summary}&rdquo;? This cannot be undone.
          </p>
          <DialogFooter>
